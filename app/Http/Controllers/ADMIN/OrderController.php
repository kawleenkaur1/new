...
class OrderController extends Controller
{
    ...
    {
        $user = DB::table('orders')->where('user_id', '$_GET[user_id]')->first();

        return view('admin.orders.search',['users'=>$users]);
    }
   
    public function search(Request $request){
        $page_limit = 10;
        $arr['users'] = User::customerUser()->get();
        $arr['deliveryboy'] = User::deliveryBoy()->get();
        if(isset($_GET['id']) && intval($_GET['id'])){
            $arr['fetchdata'] =$orders= Order::orderDesc()->where('id',$_GET['id'])
            ->paginate($page_limit);
        }
        else{

        if(isset($_GET['user_id']) && intval($_GET['user_id']) && isset($_GET['delivery_boy_id']) && intval($_GET['delivery_boy_id']) && isset($_GET['status']) && intval($_GET['status'])){
            $user_id = $_GET['user_id'];
            $arr['fetchdata'] =$orders= Order::orderDesc()->where('user_id',$user_id)
            ->where('delivery_boy_id',$_GET['delivery_boy_id'])
        ...
        elseif(isset($_GET['user_id']) && intval($_GET['user_id'])){
            $user_id = $_GET['user_id'];
            $arr['fetchdata'] =$orders= Order::orderDesc()->where('user_id',$user_id)->paginate($page_limit);

        }
        elseif(isset($_GET['delivery_boy_id']) && intval($_GET['delivery_boy_id'])){

	            $arr['fetchdata'] =$orders= Order::orderDesc()->where('delivery_boy_id',$_GET['delivery_boy_id'])->paginate($page_limit);
	
        }
        elseif(isset($_GET['status']) && intval($_GET['status'])){
	        $arr['fetchdata'] =$orders= Order::orderDesc()
            ->where('status',$_GET['status'])
            ->paginate($page_limit);
	}elseif(isset($_GET['user_id']) && intval($_GET['user_id']) && isset($_GET['delivery_boy_id']) && intval($_GET['delivery_boy_id'])){
	    $user_id = $_GET['user_id'];
        $arr['fetchdata'] =$orders= Order::orderDesc()
	...
	}elseif(isset($_GET['delivery_boy_id']) && intval($_GET['delivery_boy_id']) && isset($_GET['status']) && intval($_GET['status'])){
	...
	}elseif(isset($_GET['user_id']) && intval($_GET['user_id'])&& isset($_GET['status']) && intval($_GET['status'])){
	    $user_id = $_GET['user_id'];
            $arr['fetchdata'] =$orders= Order::orderDesc()
            ->where('user_id',$user_id)
            ->where('status',$_GET['status'])
            ...
            $arr['fetchdata'] = $orders = Order::orderDesc()->paginate($page_limit);
	    }
    if(!empty($_GET)){
        $user_id = $_GET['user_id'];
            $orders->appends (array ('user_id' => $user_id,'delivery_boy_id'=>$_GET['delivery_boy_id'],'status'=>$_GET['status']));

        }
    }
    
        return view('admin.orders.search', $arr);
    }
    ...
    public function fetch_buyonce_orders(Request $request)
    {
        $page_limit = 100;
        $template['page_title'] = 'Orders';
        ...
        if(isset($_GET['q']) && !empty($_GET['q'])){
            ...
            $user_id = $_GET['user_id'];
            $users = Order::orderDesc()->where('user_id',$user_id)->paginate($page_limit);
            $users->appends (array ('user_id' => $user_id));
        }
        else{
            $users = Order::orderDesc()->paginate($page_limit);
        }
    ...
    public function fetch_pending_orders(Request $request)
    {
        ...
        $template['page_title'] = 'Pending Orders';
          ...
          if (isset($_GET['get_export_data'])) {
             $data[] = array("ORDERID","Name","PINCODE","Address","Amount","DeliveryBoy","Subs delivery Boy","Status","order_type","Payment Mode","Payment Status","Pending Amount","Added On");
              $acounts = Order::orderDesc()->pending()->paginate();
 ...
 $users = Order::orderDesc()->where('status',1)->where('delivery_boy_id',$_GET['user_id'])->paginate($page_limit);
        $template['fetchdata'] = $users;
        return view('admin.orders.orders',$template);
    }
 ...
 $users = Order::orderDesc()->where('status',2)->where('delivery_boy_id',$_GET['user_id'])->paginate($page_limit);
        $template['fetchdata'] = $users;
        return view('admin.orders.orders',$template);
    }


    public function fetch_floting_orders(Request $request)
    {
        $page_limit = 100;
        $template['page_title'] = 'Floting Orders';

          if (isset($_GET['get_export_data'])) {
             $data[] = array("ORDERID","Name","PINCODE","Address","Amount","DeliveryBoy","Subs delivery Boy","Status","order_type","Payment Mode","Payment Status","Pending Amount","Added On");
              $acounts = Order::orderDesc()->floting()->paginate();
        ...
        $template['fetchdata'] = $users; 
        $template['fetchcollect'] = $total = Order::where('payment_mode','cod')->where('status',2)->sum('payable_amount');
        $template['fetchhandover_amount'] =$handover =  DB::table('cash_deposits')->whereIn('status',[2])->sum('amount');
        $template['fetchpending'] =  $total - $handover;
        
        // print_r( $template['fetchcollect']); die;
        return view('admin.orders.floting',$template);
    }

    public function fetch_cash_deposits(Request $request)
    {
        $page_limit = 100;
        $template['page_title'] = 'Cash Deposits Orders';
        ...
        if(isset($_GET['q']) && !empty($_GET['q'])){
            ...
            $users =DB::table('cash_deposits')->orderBy('id', 'DESC')->paginate($page_limit);
        }
        $template['fetchdata'] = $users;
        
        return view('admin.orders.cashDeposits',$template);
    }

 public function cash_deposits_verified($id)
    {
        // print_r($id); die;
        DB::table('cash_deposits')->where('id',$id)->update(['status'=>1]);
    ...
    public function fetch_completedorders(Request $request)
    {
        $page_limit = 100;
        $template['page_title'] = 'Completed Orders';
         ...
         if (isset($_GET['get_export_data'])) {
             $data[] = array("ORDERID","Name","PINCODE","Address","Amount","DeliveryBoy","Subs delivery Boy","Status","order_type","Payment Mode","Payment Status","Pending Amount","Added On");
               $acounts = Order::orderDesc()->completed()->paginate();
        ...
        if(isset($_GET['q']) && !empty($_GET['q'])){
            ...
            $users = Order::orderDesc()->completed()->paginate($page_limit);
        }
    ...
    public function fetch_cancelledorders(Request $request)
    {
        $page_limit = 100;
    ...
    public function load_order_data($id)
    {
        ...
        return View::make('admin.orders.view-details',$data)->render();
    }
    ...
    public function load_assign_deliveryBoy_data($id)
    {
        $data['deliveryBoy'] = User::deliveryBoy()->where('is_online',1)->get();
        ...
        $data['order_id']=$id;
        return View::make('admin.orders.assignDelivery',$data)->render();

    }

    public function assign_deliveryBoy(Request $request,$id)
    {
        if(empty($id)){
