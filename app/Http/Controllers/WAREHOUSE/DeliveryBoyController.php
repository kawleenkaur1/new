...
{
    public function fetch_deliveryboyswarehouse(Request $request)
    {
        $page_limit = 10;
        ...
        $id = $user->id;
           ...
           if (isset($_GET['get_export_data'])) {
            ...
            $acounts = User::where('warehouse_id',$id)->deliveryBoy()->paginate();
                $i = 1;
        ...
        if(isset($_GET['q']) && !empty($_GET['q'])){
            $q = trim($_GET['q']);
            $users = User::where('warehouse_id',$id)->where('status',1)->deliveryBoy()
            ...
            if(!empty($date)){
                ...
                if(count($arr) === 1){
                    $s_date = trim($arr[0]);
                    $users = User::where('warehouse_id',$id)->where('status',1)->deliveryBoy()->whereDate('created_at', $s_date)
                    ->paginate($page_limit);
            ...
            $users = User::where('warehouse_id',$id)->deliveryBoy()->paginate($page_limit);
        }
        $template['users'] = $users;
        return view('warehouse.users.deliveryboys',$template);
    }

public function fetch_deliveries_by_deliveryboywarehouse($value='')
{
        $page_limit = 10;
    ...
    if(isset($_GET['q']) && !empty($_GET['q'])){
        ...
        return view('warehouse.orders.delivery_boy_orders',$template);
}

    public function add_deliveryboywarehouse(Request $request)
    {
        ...
        $breadcrumb = [
          ...
          $user = Auth::user();
          $template['warehouses_id']= $user->id;


         // $template['warehouses'] = User::active()->warehouse()->get();
         $template['breadcrumb'] = $breadcrumb;
        return view('warehouse.users.add-deliveryboy',$template);
    }
    ...
    public function save_deliveryboywarehouse(Request $request)
    {
        ...
        if($create->id){
            return Redirect::route('fetch_deliveryboyswarehouse')->with('success', 'Successfully Saved!');
    ...
    public function edit_deliveryboywarehouse($id=0)
    {
        $template['page_title'] = 'Edit Delivery Boy';
        $breadcrumb = [
            0=>[ 'title'=>'Delivery Boys List',
             'link'=>route('fetch_deliveryboyswarehouse') ]
        ];
        ...
        return view('warehouse.users.edit-deliveryboy',$template);
    }


 public function changePasswordwarehouse()
    {
        $template['page_title'] = 'Edit Delivery Boy';
        $breadcrumb = [
            0=>[ 'title'=>'Delivery Boys List',
        ...
        $template['warehouses'] = User::active()->warehouse()->get();
        $template['breadcrumb'] = $breadcrumb;
        return view('warehouse.users.password_change',$template);
    ...
    public function update_deliveryboywarehouse(Request $request,$id)
    {
        $validatedData = $request->validate([
            'name' => 'required',
            'phone' => 'required|min:10|max:10',
