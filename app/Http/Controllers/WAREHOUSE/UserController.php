...
class UserController extends Controller
{
    ...
    public function get_user_detailswarehouse($id)
    {
        $data['user']= User::where('id',$id)->first();
        $data['address']=Address::where('user_id',$id)->default()->first();
        return View::make('warehouse.users.customer-view',$data)->render();
    }
    ...
    public function can_cash_hold_enablewarehouse($user_id)
    {
        User::where('id',$user_id)->update(['can_cash_hold'=>1]);
        return redirect()->back()->with('success', 'Successfully Updated!');
    }

    public function can_cash_hold_disablewarehouse($user_id)
    {
