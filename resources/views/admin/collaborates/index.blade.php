                                ...
                                {{-- <div class="widget-content widget-content-area"> --}}
                                    ...
                                    <div class="form-group mb-0 mt-2">
                                        ...
                                        @if (isset($_GET['search_date']))
                                            <a href="{{route('fetch_supports')}}" class="btn btn-sm btn-dark">Clear</a>
                                        @endif
                                ...
                                @foreach ($fetchdata as $item)
                                    ...
                                    <tr>
                                        ...
                                        $user_detail_url = route('get_user_details',['id'=>$item->user_id]);
                                            ?>
                                        <td><a style="color: red;cursor: pointer;" onclick="return get_user_details(<?=$item->user_id?>,'<?=$user_detail_url?>')" id="<?=$item->user_id?>">{{$item->user ? $item->user->name : 'User'}}</a></td>
                                        <td>{{$item->email}}</td>
                                        <td>{{$item->phone}}</td>
                                        <td>{{$item->message}}</td>
                                        <td>{{$item->type}}</td>
                                        ...
                                        <td>
                                            @if ($item->status==0)
                                                <a href="{{route('mark_as_solved_collaborates',['id'=>$item->id])}}" onclick="return confirm('Are you sure?')" class="btn btn-sm btn-warning">Mark as Solved</a>
