...
<div class="layout-px-spacing">
    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            <button class="btn btn-primary  mb-2 mr-2"data-toggle="modal" data-target="#add_vehicle_category" style="float: right">Add Subcategory</button>
        </div>
        @if ($errors->any())
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            ...
            <div class="widget-content widget-content-area br-6">
                ...
                <form  method="GET">
                    ...
                    <div class="form-group mb-0 mt-2">
                        ...
                        <a href="{{route('fetch_subcategories')}}" class="btn btn-sm btn-dark">Reset</a>
                            ...
                            <?php $i = 1; ?>
                                ...
                                @foreach ($fetchdata as $item)
                                    <tr>
                                        <td>{{$count++}}</td>
                                        <td>{{$item->name}}</td>
                                        <td><span class="badge badge-dark">{{$item->category ? $item->category->name : ''}}</span></td>

                                        <td>
                                            <div class="user-img">
                                                <img style="width:80px;" class="usr-img rounded-circle" src="{{$item->image_url}}" alt="profile">
                                            </div>
                                        </td>
                                        <td>
                                            @if ($item->status == 1)
                                        ...
                                        <td>
                                            <?php
                                            $url_ct = route('load_subcategory_data',['id'=>$item->id]);
                                                ?>
                                            <button class="btn btn-sm btn-primary mb-2 mr-2" onclick="return load_data_for_edit(<?=$item->id?>,'<?=$url_ct?>')" id="<?=$item->id?>">
                                                Edit</button>
                                            <a class="btn btn-sm btn-danger mb-2 mr-2" onclick="return confirm('Are you sure ?')" href="{{route('delete_subcategory',['id'=>$item->id])}}">
                                                Delete</a>
