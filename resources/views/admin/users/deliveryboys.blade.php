...
<div class="layout-px-spacing">
    ...
    <div class="row layout-top-spacing">
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
            <a class="btn btn-primary  mb-2 mr-2" href="{{route('add_deliveryboy')}}" style="float: right">Add Delivery Boy</a>
        </div>
        @if ($errors->any())
        <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th>Pincode</th>
                                      ...
                                      <th>Pending Floting Cash</th>
                                    <th>Clear Pending Cash</th>
                                ...
                                @foreach ($users as $user)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center"> 
                                           ...
                                           <a href="{{route('can_cash_hold_disable',['id'=>$user->id])}}" onclick="return confirm('Are you sure?')" ><i class="fas fa-toggle-on"></i></a>
                                       @else
                                        <a href="{{route('can_cash_hold_enable',['id'=>$user->id])}}" onclick="return confirm('Are you sure?')" ><i class="fas fa-toggle-off"></i></a>
                                       @endif
                                        ...
                                        <td>  
                                            <?php
                                            $d_id =   $user->id;
                                            $cashcollects = DB::table('cashcollects')->where('status',0)->where('deliveryboy_id',$d_id)->sum('amount');
                                              if ($cashcollects) {
                                                echo $cashcollects;
                                              }
                                              else{
                                        ...
                                        <td> <?php
                                            $d_id =   $user->id;
                                            $cashcollects = DB::table('cashcollects')->where('status',0)->where('deliveryboy_id',$d_id)->sum('amount');
                                              if (!empty( $cashcollects )) {
                                               ?>
                                                 <a href="{{route('cash_PayOut_deliveryboys',['id'=>$user->id])}}" onclick="return confirm('Are you sure?')" ><button type="button" class="btn btn-primary btn-sm">PayOut</button></a>
                                               <?php
                                              }
                                              else{
                                                ?>
                                                <button type="button" class="btn btn-success btn-sm">Clear</button></a>
                                                <?php
                                               
                                              }
                                            ?></td>
                                            <td class="text-center">
                                                <a style="color: red" href="{{route('pending_order')}}?user_id=<?=$user->id?>">{{$user->deliveryboyorders ? $user->deliveryboyorders->whereIn('status',[0,1])->count() : 0}}</a>
                                            </td>
                                            <td class="text-center">
                                            <a style="color: red" href="{{route('order_delivered')}}?user_id=<?=$user->id?>">{{$user->deliveryboyorders ? $user->deliveryboyorders->where('status',2)->count() : 0}}</a>
                                            </td>
                                        ...
                                        <td class="text-center">
                                            ...
                                            $user_detail_url = route('get_user_details',['id'=>$user->id]);
                                                ?>
                                            <a class="mb-2 mr-2" href="{{route('edit_deliveryboy',['id'=>$user->id])}}" ><i class="fas fa-edit"></i></a>
