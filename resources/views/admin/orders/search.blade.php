...
<div class="layout-px-spacing">
  <div class="row layout-top-spacing">
    ...
    <form action="search" method="get">
      <div class="row">
      <select class="form-select col-md-2" name="user_id" aria-label="Default select example">
        <option selected>Select Customers</option>
         @foreach($users as $country)
          <option value="{{$country->id}}"{{(isset($_GET['user_id']) && $_GET['user_id']==$country->id)  ? 'selected' : ''}}>{{$country->name}}</option>
        @endforeach</select>
      </select>
      <select class="form-select col-md-2" name="delivery_boy_id" aria-label="Default select example">
        <option selected>Select Delivery Boy</option>
         @foreach($deliveryboy as $country)
          <option value="{{$country->id}}"{{(isset($_GET['delivery_boy_id']) && $_GET['delivery_boy_id']==$country->id)  ? 'selected' : ''}}>{{$country->name}}</option>
        @endforeach 
      </select>
      <select class="form-select col-md-2" name="status" aria-label="Default select example">
      ...
      <div>
      <input type="text"  name="id" value="<?=isset($_GET['id']) ? $_GET['id'] : '' ?>" placeholder="Search Order_Id">
      </div>
      <div>
        <input id="rangeCalendarFlatpickr" value="<?=isset($_GET['search_date']) ? $_GET['search_date'] : '' ?>" name="search_date"  type="text" placeholder="Select Date..">
        </div>
                            ...
                            <div class="dropdown custom-dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                    ...
                                    <?php if (($item->status == 0 || $item->status == 1)) : ?>
                                        ...
                                        <a style="cursor: pointer" class="dropdown-item" href="{{route('delivered_order',['id'=>$item->id])}}" onclick="return confirm('are you sure you have delivered the order?')">
                                                Complete</a>
                                    <?php endif; ?>
