...
const fetch_product_details = async (req,res) => {
    ...
    if(product){
        ...
        product.dataValues.is_wishlist = checkifadded?1:0;
...
const fetch_combos = async (req,res) => {
    ...
    const combos = await Product.scope(['combos','active']).findAll().then(async (produs)=>{
        for (let product of produs) {
            ...
            product.dataValues.is_cart = 0
            product.dataValues.cartdata = null
    ...
    res.json(combos ? successRes('fetched',combos):failedRes('failed'));
}
